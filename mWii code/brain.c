...
int findBlob(irCam* blobs) {
	int i; int bright=0;
	for(i=0; i<4; i++) {
		if((blobs[i].size>bright)&&(blobs[i].size!=15))
			bright=blobs[i].size;
	}
...
{	
	...
	char check=0;
	irCam Blobs[4];
	int which_blob, dir, rot=0; // Direction of LED, Rotation of servo
	...
	while(1){		
		...
		which_blob=0; // Index no. of brightest blob
		dir=Blobs[which_blob].x;
		
		/* Start relaying the data over USB */
		if(m_usb_isconnected()) {
			if(check) {					
				m_usb_tx_string("Blob ");
				m_usb_tx_int(which_blob);
				m_usb_tx_string(": ");
				m_usb_tx_int(Blobs[which_blob].x);
				m_usb_tx_string(" | ");
